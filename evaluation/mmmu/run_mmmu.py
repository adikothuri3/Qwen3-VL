...
from dataset_utils import MMMU_preproc, dump_image, load_dataset
    ...
    if args.use_cot:
        cot_prompt = (
            ...
            if args.cot_prompt
                ...
                " based on the information provided. Avoid repeating steps"
                " indefinitely—provide your best guess even if unsure. Determine whether"
                " to think step by step based on the difficulty of the question,"
...
def run_evaluation(args):
    ...
    # Validate: each data.iloc[i].index must exist in results and
    # data.iloc[i].question must match results[i]['annotation'].question
    print(f"len(data): {len(data)}")
    print(f"len(meta): {len(meta)}")
    meta_q_map = {x: y for x, y in zip(meta["index"], meta["question"])}
    data_map = {x: y for x, y in zip(data["index"], data["question"])}
    for k in data_map:
        assert k in meta_q_map, "eval_file should be the same as or a subset of dataset MMMU_DEV_VAL"
    ...
    if debug:
        ...
        with ThreadPoolExecutor(max_workers=args.nproc) as executor:
            for result in tqdm(executor.map(eval_single_sample, eval_tasks), total=len(eval_tasks), desc="Evaluating"):
                eval_results.append(result)
    ...
    for split, split_results in results_by_split.items():
        ...
        hits = sum(r["hit"] for r in split_results)
        total = len(split_results)
        print(f"Accuracy for {split} split: {split_accuracy:.4f} ({hits}/{total})")
    ...
    with open(args.output_file.replace(".csv", "_acc.json"), "w") as f:
        json.dump({"overall_accuracy": accuracy, "accuracy_by_split": accuracy_by_split}, f, indent=2)
...
def main():
    ...
    eval_parser.add_argument(
        "--api-type", type=str, default="dash", choices=["dash", "mit"], help="API type to use for evaluation"
    )
