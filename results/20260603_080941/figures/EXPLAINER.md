...
- **Samples:** 8  (sources: ['url', 'url', 'url', 'url', 'url', 'url', 'url', 'url'])
- **Groups:** 3 — visual features tapped at ViT layers [5, 11, 17] and injected into decoder layers [0, 1, 2].
- **Visual tokens per image:** [672, 300, 672, 260, 260, 704, 736, 64]
...
How much attention each group's tokens **receive** from the rest of the sequence (averaged over heads and query positions). Tokens that get more attention are relied upon more, so a group whose tokens are widely ignored is more compressible.

## Takeaway from this run
- Weakest group on average: **Group 0** (ViT L5, mean strength 14.6) → most prunable.
- Strongest group on average: **Group 2** (ViT L17, mean strength 23.6) → most fragile, keep more of it.
